...
Object.keys(locData).forEach(locale => {
    ...
    const jsonpContent = `IntlPolyfill.__addLocaleData(${JSON.stringify(
        obj
    ).replace(jsonpExp, '$1:')});`;
...
objs.forEach((val, idx) => {
    const ref = JSON.stringify(val).replace(
        /"###(objs|prims)(\[[^#]+)###"/g,
        replacer
    );
...
console.log(
    'Total number of reused strings is ' +
        prims.length +
        ' (reduced from ' +
        valCount +
        ')'
...
console.log(
    'Total number of reused objects is ' +
        Object.keys(objStrs).length +
        ' (reduced from ' +
        objCount +
        ')'
);
